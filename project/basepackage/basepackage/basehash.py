...
class ServerHash(HashRing):
            ...
            try:
                ...
                if rsp.status_code == 200:
                    ...
                    if self._nodeStatus[node]:
                        ...
                        self._globalLog.getInfo().log(
                            self._globalLog.ERROR,
                            "Disable node %s still down." %
                            node)
            except Exception as msg:
                exc_type, _, exc_tb = sys.exc_info()
                self._globalLog.getError().log(
                    self._globalLog.ERROR, "Monitor reconnect failed. Exception type"
                    ":%s, msg:%s" %
                    (exc_type, msg))
    ...
    def _post(self, url, timeout=1, data=None, **kwargs):
        ...
        if rsp is None:
            self._globalLog.getInfo().log(
                self._globalLog.ERROR,
                "Rsp is None when post requests, url:%s" %
                (url))
        elif rsp.status_code != 200:
            self._globalLog.getInfo().log(
                self._globalLog.ERROR, "Post data to hbase failed, url:%s, rsp:%d" %
                (url, rsp.status_code))
        return rsp
    ...
    def post(self, url, timeout=1, data=None, **kwargs):
        ...
        for index in range(nodeLen):
            ...
            try:
                ...
                exc_type, _, exc_tb = sys.exc_info()
                self._globalLog.getError().log(
                    self._globalLog.ERROR, "Post request failed. Exception type"
                    ":%s, url:%s, msg:%s" %
    ...
    def _get(self, url, timeout=5, **kwargs):
        ...
        if rsp is None:
            self._globalLog.getInfo().log(
                self._globalLog.DEBUG,
                "Rsp is None when get requests, url:%s" %
                (url))
        elif rsp.status_code != 200:
            self._globalLog.getInfo().log(
                self._globalLog.DEBUG, "Get data from hbase failed, url:%s, rsp:%d" %
                (url, rsp.status_code))
        return rsp
    ...
    def get(self, url, timeout=5, **kwargs):
        ...
        for index in range(nodeLen):
            ...
            try:
                ...
                exc_type, _, exc_tb = sys.exc_info()
                self._globalLog.getError().log(
                    self._globalLog.ERROR, "Get request failed. Exception type"
                    ":%s, url:%s, msg:%s" %
                    (str(exc_type), urlstr, msg))
        self._globalLog.getError().log(self._globalLog.ERROR,
    ...
    def _put(self, url, data=None, timeout=5, **kwargs):
        ...
        if rsp is None:
            self._globalLog.getInfo().log(
                self._globalLog.ERROR,
                "Rsp is None when put requests, url:%s" %
                (url))
        elif rsp.status_code != 200:
            self._globalLog.getInfo().log(
                self._globalLog.ERROR, "Http response unusual, url:%s, rsp:%d" %
                (url, rsp.status_code))
        return rsp
    ...
    def put(self, url, data=None, timeout=5, **kwargs):
        ...
        for index in range(nodeLen):
            ...
            try:
                ...
                exc_type, _, exc_tb = sys.exc_info()
                self._globalLog.getError().log(
                    self._globalLog.ERROR, "Put request failed. Exception type"
                    ":%s, url:%s, msg:%s" %
                    (str(exc_type), urlstr, msg))
        self._globalLog.getError().log(self._globalLog.ERROR,
                                       "Occur unknown error when call post,"
